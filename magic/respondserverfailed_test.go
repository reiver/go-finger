...
func TestRespondServerFailed(t *testing.T) {
	...
	magicfinger.RespondServerFailed(rw, request)

	var actual   string = conn.String()
	var expected string =
		"\xEF\xBB\xBF"+
		"Magic-Finger"                                +"\r\n"+
		"!SERVER-FAILED {/GET joeblow/something.txt}" +"\r\n"+
		""                                            +"\r\n"

	if expected != actual {
		t.Errorf("The actual magic-finger server-failed response is not what was expected.")
